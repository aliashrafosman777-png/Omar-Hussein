...
/**
 * Shared animated gradient blobs background.
 *
 * Renders 3 blurred radial-gradient circles that drift between random positions
 * using the Web Animations API (WAAPI).
 * - Crimson blob
 * - Deep-blue blob
 * - Burgundy blob
 *
 * Respects `prefers-reduced-motion` — shows a static position.
 * All Math.random calls happen inside useEffect (client-only) to avoid
 * hydration mismatches.
 *
 * @param className  - Additional CSS classes on the outer container.
 * @param opacity    - Master opacity multiplier (0–1). Defaults to 1.
 * @param sizeScale  - Scale factor for blob sizes. Defaults to 1.
 * @param style      - Additional inline styles on the outer container.
 */
...
export interface AnimatedGradientBlobsProps {
  /** Additional CSS classes on the outer container */
  className?: string;
  /** Master opacity multiplier (0–1). Defaults to 1. */
  opacity?: number;
  /** Scale factor for blob sizes. Defaults to 1. */
  sizeScale?: number;
    ...
    >
      {BLOBS.map((blob, i) => (
        <div
          ...
          style={{
            position: "absolute",
            width: blob.size * sizeScale,
            height: blob.size * sizeScale,
            background: blob.bg,
            borderRadius: "50%",
            opacity: blob.opacity * opacity,
            willChange: "transform",
            // Initial position: center of range (deterministic for SSR)
